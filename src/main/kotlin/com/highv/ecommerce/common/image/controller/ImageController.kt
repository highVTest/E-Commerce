...
class ImageController(
    ...
    ): ResponseEntity<ImageUrlResponse> = ResponseEntity.ok(imageService.uploadImage(file, user?.id))
    ...
    fun uploadImages(
        ...
        @AuthenticationPrincipal user: UserPrincipal?
    ): ResponseEntity<List<ImageUrlResponse>> = ResponseEntity.ok(imageService.uploadImages(files, user?.id))
}
