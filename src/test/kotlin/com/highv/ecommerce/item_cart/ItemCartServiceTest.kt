...
class ItemCartServiceTest : BehaviorSpec() {
    ...
    init {
        ...
        val buyerRepository = mockk<BuyerRepository>(relaxed = true)
        val itemCartService: ItemCartService = ItemCartService(itemCartRepository, productRepository, buyerRepository)
        ...
        this.Given("상품을 장바구니에 담을 때") {
            ...
            When("상품 수량이 1개 이상이면") {
                ...
                Then("장바구니에 추가된다.") {
                    ...
                    verify(exactly = 1) { buyerRepository.findByIdOrNull(buyerId) }
            ...
            When("담는 수량이 1개 이상이고 이미 장바구니에 있는 상품이면") {
                ...
                every {
                    itemCartRepository.findByProductIdAndBuyerId(
                        product.id!!,
                        buyerId
                    )
                } returns itemCart.apply { id = 1L }

                every { itemCartRepository.save(any()) } returns itemCart.apply { quantity += request.quantity }

                Then("상품 개수가 추가된다.") {
                    val result = itemCartService.addItemIntoCart(product.id!!, request, buyerId)

                    result.msg shouldBe "장바구니에 상품이 추가됐습니다."
                }
        ...
        this.Given("장바구니에 담긴 상품의 개수를 변경할 때") {
            ...
            When("변경할 수량이 1개 이상이면") {
                ...
                every { itemCartRepository.updateQuantityByProductIdAndBuyerId(any(), any(), any()) } returns Unit

                Then("수량이 변경된다.") {

                    val response = itemCartService.updateItemIntoCart(productId, request, buyerId)
            ...
            When("변경할 수량이 1개 미만이면") {
                val request: SelectProductQuantity = SelectProductQuantity(quantity = 0)

                Then("상품이 없다는 예외가 발생한다.") {
                    shouldThrow<InvalidQuantityException> {
                        itemCartService.updateItemIntoCart(productId, request, buyerId)
                    }.let {
                        it.message shouldBe "상품의 수량이 1개보다 적을 수 없습니다."
                    }
                }
        ...
        this.Given("구매자가 장바구니에서") {
            val buyerId = 1L

            val productId = 1L

            When("상품을 삭제하면") {
                every { itemCartRepository.deleteByProductIdAndBuyerId(any(), any()) } returns Unit

                Then("상품이 삭제된다.") {
                    itemCartService.deleteItemIntoCart(productId, buyerId)
                }
            }
